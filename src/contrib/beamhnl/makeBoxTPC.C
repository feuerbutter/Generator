...
void makeBoxTPC()
{
    ...
    const double TPCSideX = 850/2, TPCSideY = 1960/2, TPCSideZ = 563/2; // m
    //--- origin is at centre of the box
    TGeoVolume * box = geom->MakeBox( "BOX", Al,
				      uMult * TPCSideX, uMult * TPCSideY, uMult * TPCSideZ );
    box->SetLineColor(kRed);
    // TGeoTranslation * tr0 = new TGeoTranslation( 0.0, 0.0, 0.0 );
    TGeoTranslation * tr1 = new TGeoTranslation( uMult*-445.0,uMult*50.0, uMult*-443.5 );
    TGeoTranslation * tr2 = new TGeoTranslation( uMult*445.0, uMult*50.0, uMult*-443.5);
    TGeoTranslation * tr3 = new TGeoTranslation( uMult*-445.0, uMult*50.0, uMult*915.5 );
    TGeoTranslation * tr4 = new TGeoTranslation( uMult*445.0, uMult*50.0, uMult*915.5 );
    TGeoTranslation * tr5 = new TGeoTranslation( uMult*-445.0, uMult*50.0, uMult*2274.5 );
    TGeoTranslation * tr6 = new TGeoTranslation( uMult*445.0, uMult*50.0, uMult*2274.5 );
    TGeoRotation * rot0 = new TGeoRotation( "rot0", thx, phx, thy, phy, thz, phz );

    //--- add directly to top volume
    // topvol->AddNode( box, 1, rot0 );
    topvol->AddNode( box, 1, tr1 );
    topvol->AddNode( box, 1, tr2 );
    topvol->AddNode( box, 1, tr3 );
    topvol->AddNode( box, 1, tr4 );
    topvol->AddNode( box, 1, tr5 );
    topvol->AddNode( box, 1, tr6 );

    //--- export this to a file
    geom->Export("./TPC.root");
